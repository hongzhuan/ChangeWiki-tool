          ...
          <ul class="tree-list">
            ...
            <li v-for="(item, index) in treeData" :key="index" class="parent-node">
              ...
              <div v-if="item.label === '项目逆向结果层次图' && item.expanded" class="subtree">
                ...
                <div v-if="selectedItem && selectedItem.type" class="child-detail">
                  <div class="meta-info">
                    <div><strong>版本：</strong>{{ selectedItem.version }}</div>
                  ...
                  >
                     <template #default="{ data }">
                      <span class="custom-tree-node">
                        {{ data.label }}
                        <span
                          v-if="getChangeSymbol(data.changeColor)"
                          class="change-indicator"
                          :class="getChangeColorClass(data.changeColor)"
                        >
                          {{ getChangeSymbol(data.changeColor) }}
                        </span>
                      </span>
                    </template>
                  </el-tree>
      ...
      treeData: [
        {
          ...
          horizontal: true,
          children: [
            { label: 'V1层次结构图', icon: 'icon-file', expanded: false, type: 'v1', isTree: true, treeData: [], version: EventBus.version1, time: '2025-06-24'},
            { label: 'V2层次结构图', icon: 'icon-file', expanded: false, type: 'v2', isTree: true, treeData: [], version: EventBus.version2, time: '2025-06-22'}
          ]
        },
        {
          label: '变更分析结果',
  ...
  methods: {
    ...
    async loadMarkdown(node) {
      ...
      try {
          ...
          // // 处理 commitId（假设 commitId 是 7 位或更多的十六进制字符串）
          // content = content.replace(/\b[a-f0-9]{7,}\b/gi, (match) => {
          //   return `<a href="#" class="md-link commit-id" data-id="${match}">${match}</a>`;
          // });

          // // 处理函数名（假设函数名是以字母开头的单词，后面跟着括号）
          // content = content.replace(/\b(\w+)\(\)/g, (match, p1) => {
          //   return `<a href="#" class="md-link function-name" data-name="${p1}">${match}</a>`;
          // });

          // // 处理文件名（假设文件名包含扩展名，如 .js、.py 等）
          // content = content.replace(/\b[\w\-]+\.(js|py|java|cpp|ts|html|css)\b/gi, (match) => {
          //   return `<a href="#" class="md-link file-name" data-file="${match}">${match}</a>`;
          // });
    ...
    async selectItem(item) {
      ...
      if (item.label === '架构变更报告') {
        try {
          const response = await axios.get('http://localhost:5000/get_markdown_architecture_change_report');
          this.mdContent = response.data.content;
          this.renderedContent = this.markdownIt.render(this.mdContent);
        } catch (e) {
          this.mdContent = '';
        ...
        try {
          // 架构变更视图：直接显示组件
          const response = await axios.get('http://localhost:5000/get_markdown_architecture_version_summary_report');
          this.mdContent = response.data.content;
          this.renderedContent = this.markdownIt.render(this.mdContent);
        } catch (e) {
          this.mdContent = '';
...
.tree-container {
  padding: 10px 0;
  max-height: calc(100vh - 20px); /* 限制最大高度，避免内容溢出 */
