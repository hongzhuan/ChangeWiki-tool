        ...
        <el-card>
          <div class="entity">
            ...
            <el-button type="text" icon="el-icon-refresh" size="mini"
              style="background-color:#2973B2; color: white; font-size: 1px; font-weight: bold;margin: 0 1%"
            ...
            <PlantUMLRight v-if="isDialogVisible" @close="isDialogVisible = false" /> 
            <div id="networkright" style="margin-top: 1%"></div>
          </div>
...
import PlantUMLRight from "@/components/PlantImgRight.vue";
export default {
  /* eslint-disable */
  components: { PlantUMLRight  },
  name: 'Architecture',
  data() {
    return {
  ...
  methods: {
    ...
    async startClustering() {
      ...
      this.setActiveComponent('reverse')
      // const response = await fetch('http://localhost:5000/run_clustering_right', {
      //   method: 'POST',
      //   headers: {
      //     'Content-Type': 'application/json',
      //   },
    ...
    getEntityAgain() {
      ...
      cytoscape.use(fcose);
      let cy = cytoscape({
        container: document.getElementById('networkright'),
        ready: function () {
