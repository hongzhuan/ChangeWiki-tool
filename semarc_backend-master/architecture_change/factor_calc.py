...
def fun_a2a_zyr(result, result_gt):
    ...
    mto=0
    for k in removeA:
        print("removeA:",result_clusters[k])
        mto += len(result_clusters[k])
        mto += 1
    for k in addB:
        print("addB:", result_clusters_gt[k])
        mto += len(result_clusters_gt[k])
        mto += 1
    for (k, k_gt) in moveAB:
        print(f"{k},{k_gt} moveAB:")
        print(result_clusters[k])
        print((len(result_clusters[k]) - edge_dict[(k,k_gt)]),(len(result_clusters_gt[k_gt]) - edge_dict[(k,k_gt)]))

        mto += (len(result_clusters[k]) - edge_dict[(k,k_gt)])
        mto += (len(result_clusters_gt[k_gt]) - edge_dict[(k,k_gt)])
        pass
...
def compare_two_cluster_results(result_dict1, result_dict2, metric_names:Iterable[str]=None)->dict:
    ...
    deletelist = [fn for fn in result_dict1 if fn not in result_dict2] #result_dict内容为文件名：簇号
    addlist = [fn for fn in result_dict2 if fn not in result_dict1] #result_dict内容为文件名：簇号
    print(deletelist)
    print(addlist)
    print(filelist)
...
def get_cluster_mapping(v1_path, v2_path):
    from collections import defaultdict
    """
    构造 v2 的 cluster 映射到 v1 的 cluster，依据文件交集最大原则。

    参数:
        v1_file_to_cluster: dict[str, int]  # 文件路径 → v1 的簇编号
        v2_file_to_cluster: dict[str, int]  # 文件路径 → v2 的簇编号

    返回:
        dict[int, int]  # 版本2的 cluster_id 映射到 版本1的 cluster_id
    """
    v1_file_to_cluster=json2cluster_dict(v1_path)
    v2_file_to_cluster=json2cluster_dict(v2_path)

     # 1. 构建 cluster_id → 文件集合
    v1_clusters = defaultdict(set)
    v2_clusters = defaultdict(set)

    for file, cid in v1_file_to_cluster.items():
        v1_clusters[cid].add(file)
    for file, cid in v2_file_to_cluster.items():
        v2_clusters[cid].add(file)

    # 2. 构建候选匹配项列表：[(v2_id, v1_id, overlap_count)]
    candidate_matches = []
    for cid_v2, files_v2 in v2_clusters.items():
        for cid_v1, files_v1 in v1_clusters.items():
            overlap = len(files_v1 & files_v2)
            if overlap >= len(files_v2) / 2:
                candidate_matches.append((cid_v2, cid_v1, overlap))

    # 3. 匹配：按照 overlap 降序，确保唯一匹配（贪心策略）
    candidate_matches.sort(key=lambda x: -x[2])  # 按 overlap 倒序排序

    used_v1 = set()
    used_v2 = set()
    mapping = {cid_v2: -1 for cid_v2 in v2_clusters.keys()}  # 初始化为 -1

    for cid_v2, cid_v1, _ in candidate_matches:
        if cid_v2 not in used_v2 and cid_v1 not in used_v1:
            mapping[cid_v2] = cid_v1
            used_v2.add(cid_v2)
            used_v1.add(cid_v1)
...
def main():
    ...
    # 解析 RSF 文件
    result_dict = json2cluster_dict("D:\\SemArc_backend\\results\\libuv-v1.44.2\\libuv-v1.44.2_NamedClusters.json")
    # 将 JSON 文件转换为集群字典
    gt_dict = json2cluster_dict("D:\\SemArc_backend\\results\\libuv-v1.48.0\\libuv-v1.48.0_NamedClusters.json")
    # 比较两个集群结果
...
# def main():
#     # 解析 RSF 文件
#     v1 = "D:\\SemArc_backend\\results\\libuv-v1.44.2\\libuv-v1.44.2_NamedClusters.json"
#     # 将 JSON 文件转换为集群字典
#     v2 ="D:\\SemArc_backend\\results\\libuv-v1.48.0\\libuv-v1.48.0_NamedClusters.json"
#     # 比较两个集群结果
#     mapping=get_cluster_mapping(v1,v2)
#
#     print(mapping)
