...
def cluster_project(
    ...
    for prj_ind in range(num_prj):
        try:
            ...
            # # Step 3: Remove files from cluster_result that are not in file_to_component
            # # Only keep files that exist in both file_to_component and cluster_result
            # cluster_result = {file: cluster for file, cluster in cluster_result.items() if file in file_to_component}

            print("————————————————————————————————————————————\n", cluster_result)

            # Step 4: Count how many files each component is mapped to per cluster
            ...
            # Step 5: Remove empty clusters (clusters with no components)
            cluster_component_count = {cluster: component_count for cluster, component_count in
                                       cluster_component_count.items() if component_count}

            # Step 6: Map each cluster to the component with the most files
