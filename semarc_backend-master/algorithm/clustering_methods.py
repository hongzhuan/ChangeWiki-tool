...
def clustering_method_dep_lda_func_weight(
    ...
    logger.info("Dependency graph loaded.")
    # cache?
    ...
    #debug:mksh
    print("result_dep_dict:",result_dep_dict)
    ...
    if USE_LDA:
        ...
        #debug:mksh
        print("result_lda_dict:",result_lda_dict)
    else:
    ...
    if pattern_file == None:
        ...
        # 优化锚点聚类逻辑
        logger.info("开始执行优化后的锚点聚类...")

        # 初始化结果存储
        clusters = defaultdict(list)  # 存储每个组件对应的文件列表
        file_to_component = {}  # 存储每个文件对应的组件

        # 每个 component 直接选择与其余弦相似度最高的文件
        remaining_files = set(range(len(file_vectors)))  # 未分配的文件索引集合

        while remaining_files:
            for component_idx in range(len(component_names)):  # 按组件遍历
                if not remaining_files:
                    break  # 如果所有文件都已分配，退出循环

                # 获取当前组件的所有 indicators 向量
                component_indicators = component_vectors[component_idx * 3:(component_idx + 1) * 3]

                # 计算当前组件与所有未分配文件的相似度（取 indicators 的平均相似度）
                similarities = np.mean(
                    cosine_similarity(file_vectors[list(remaining_files)], component_indicators),
                    axis=1
                )

                # 找到与当前组件相似度最高的文件
                best_file_idx = list(remaining_files)[np.argmax(similarities)]

                # 分配文件到组件
                clusters[component_names[component_idx]].append(file_names[best_file_idx])
                file_to_component[file_names[best_file_idx]] = component_names[component_idx]

                # 从未分配文件集合中移除该文件
                remaining_files.remove(best_file_idx)

        print("锚点聚类完成，结果如下:")
        for component, files in clusters.items():
            print(f"组件 {component}: {len(files)} 个文件")
        # 保存聚类结果到JSON文件
