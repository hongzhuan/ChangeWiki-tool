...
def compare_two_cluster_results(result_dict1, result_dict2, metric_names:Iterable[str]=None)->dict:
    # 新增：完全相同直接返回全1
    if result_dict1 == result_dict2:
        if metric_names is None:
            metric_names = METRICS_TO_FUNCTION
        return {metric: 1 for metric in metric_names}
...
def main():
    # 解析 RSF 文件
    v1 = "D:\\SemArc_backend\\results\\libuv_new-v1.44.2\\libuv_new-v1.44.2_NamedClusters.json"
    # 将 JSON 文件转换为集群字典
    v2 ="D:\\SemArc_backend\\results\\libuv_new-v1.48.0\\libuv_new-v1.48.0_NamedClusters.json"
    # 比较两个集群结果
