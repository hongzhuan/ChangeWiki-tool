...
# ollama_endpoint = "http://10.180.162.140:11434/api/chat" #本地大模型
ollama_endpoint = "http://127.0.0.1:6006/api/chat"  #gpu服务器大模型
...
if deepseek_endpoint in API_URL_REDIRECT: deepseek_endpoint = API_URL_REDIRECT[deepseek_endpoint]  # siliconflow
if deepseekapi_endpoint in API_URL_REDIRECT: deepseekapi_endpoint = API_URL_REDIRECT[
    deepseekapi_endpoint]  # official deepseek
...
siliconflow_noui, siliconflow_ui = get_predict_function(
    api_key_conf_name="API_KEY", max_output_token=512, disable_proxy=False
)
model_info = {
    # openai
    "gpt-3.5-turbo": {
...
# -=-=-=-=-=-=- siliconflow（deepseek） 对齐支持 -=-=-=-=-=-=-
if "deepseek-ai/DeepSeek-R1" in AVAIL_LLM_MODELS:
    try:
        siliconflow_noui, siliconflow_ui = get_predict_function(
            api_key_conf_name="API_KEY", max_output_token=512, disable_proxy=False
        )
    ...
    if '&' not in model:
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                for i in range(n_model):
                    chat_string.append(
                        f"【{str(models[i])} 说】: <font color=\"{colors[i]}\"> {window_mutex[i][0]} </font>")
                res = '<br/><br/>\n\n---\n\n'.join(chat_string)
                # # # # # # # # # # #
        ...
        for i, future in enumerate(futures):  # wait and get
            return_string_collect.append(
                f"【{str(models[i])} 说】: <font color=\"{colors[i]}\"> {future.result()} </font>")
