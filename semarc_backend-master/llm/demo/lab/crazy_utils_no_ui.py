...
def request_gpt_model_multi_threads_with_no_ui_and_high_efficiency(
    ...
    total_requests = len(inputs_array)
    total_retries = 0
    total_failures = 0
    ...
    def _req_gpt(index, inputs, history, sys_prompt):
        nonlocal total_retries, total_failures
        gpt_say = ""
        retry_op = retry_times_at_unknown_error
        exceeded_cnt = 0
        mutable[index][2] = "执行中"
    ...
    failure_ratio = total_failures / total_requests if total_requests > 0 else 0
    log_entry = (
        f"开始时间: {start_time}, 结束时间: {end_time}, 多线程数: {max_workers}, "
        f"总请求次数: {total_requests}, 重试次数: {total_retries}, 重试占比: {retry_ratio:.2%}, "
        f"失败次数: {total_failures}, 失败占比: {failure_ratio:.2%}\n"
    )
    log_file = os.path.join(get_log_folder(), "llm_req_log.txt")
...
def can_multi_process(llm):
    ...
    if llm.startswith('ollama-'): return True #允许ollama多线程
    if llm.startswith('deepseek-'): return True #支持deepseek多线程
    return False
